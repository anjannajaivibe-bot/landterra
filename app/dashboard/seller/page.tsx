...
export default function SellerDashboardPage() {
  ...
  const draftsCount = properties.filter(
    (p) => p.listingStatus === 'DRAFT' || p.listingStatus === 'PAYMENT_PENDING'
  ).length;
  const inactiveCount = properties.filter((p) => p.listingStatus === 'EXPIRED').length;
  ...
  const filteredProperties = properties.filter((p) => {
    ...
    if (statusFilter === 'DRAFT')
      return p.listingStatus === 'DRAFT' || p.listingStatus === 'PAYMENT_PENDING';
    if (statusFilter === 'INACTIVE') return p.listingStatus === 'EXPIRED';
      ...
      <div className="bg-slate-900 text-white py-8 border-b border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-black tracking-tight">Property Portfolio Management</h1>
            <p className="text-xs text-slate-400 mt-1">
              Manage your property listings, review enquiries and keep published information up to date.
            </p>
          </div>
                ...
                {loading ? (
                  ...
                  <div className="divide-y divide-slate-100">
                    {filteredProperties.map((prop) => {
                      const isDraft =
                        prop.listingStatus === 'DRAFT' || prop.listingStatus === 'PAYMENT_PENDING';
                      const isInactive = prop.listingStatus === 'EXPIRED';
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="space-y-1">
                              ...
                              {isDraft && (
                                <p className="text-[11px] text-amber-700 font-medium">
                                  Draft saved. Complete the required details and submit it for review.
                                </p>
                              )}
                          ...
                          <div className="flex items-center gap-2 self-end md:self-center flex-wrap">
                            ...
                            {/* Legacy inactive listing action */}
                            {isInactive && (
                              <Link
                                href={`/sell?propertyId=${prop._id}`}
                                className="inline-flex items-center gap-1 px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-semibold transition-colors"
